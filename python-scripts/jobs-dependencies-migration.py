...
west_us_cluster_ids = ["1234556", "1234523253", "235224"]
...
for west_us_cluster_id in west_us_cluster_ids:

    print("Currently Migrating cluster ID: ",west_us_cluster_id)
    ...
    if west_us_response.status_code == 200:
        ...
        if create_response.status_code == 200:
            print(f"{west_us_cluster_id} created successfully in West US 2 workspace!")
        else:
            print(f"Failed to create {west_us_cluster_id} in West US 2: {create_response.text}")
    else:
        print(f"Failed to fetch {west_us_cluster_id} details from West US workspace: {west_us_response.text}")